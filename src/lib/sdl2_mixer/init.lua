-- LuaJIT binding for SDL2_mixer library
-- Binding made by +KZ for PLUSKAIZO

local ffi = require 'ffi'
local lib = ffi.load("SDL2_mixer")
...
--code taken from src/external/sdl2_image.lua:

local M = {C=lib}

setmetatable(M,{
__index = function(t,k)
	local ok,ptr = pcall(function(str) return lib["Mix_"..str] end,k)
	...
	if not ok then --torch sdl2 calling
		local str2 = "Mix_"..string.upper(k:sub(1,1))..k:sub(2)
