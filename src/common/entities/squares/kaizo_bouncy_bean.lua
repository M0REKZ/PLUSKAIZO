...
KaizoBouncyBean = setmetatable({}, {__index = KaizoSquare})

KaizoBouncyBean.name = "KaizoBouncyBean"

KaizoBouncyBean.__index = KaizoBouncyBean

KaizoBouncyBean.editor_properties = {}
KaizoBouncyBean.editor_properties[1] = "dir"


function KaizoBouncyBean:new(x,y)
    local o = KaizoSquare:new(x,y,32,32)
    o = setmetatable(o,KaizoBouncyBean)
...
function KaizoBouncyBean:update()
    ...
    if not self.jumped and self.grounded then
        self.vel.y = -15
        self.jumped = true
    end
...
function KaizoBouncyBean:render()
    ...
    if(self.image) then
		...
		if(self.dir < 0) then
            ...
            self.image:render_incamera_scaled_from_to(0, self.size.y * self.frame, self.size.x, self.size.y, self.pos.x + self.size.x , self.pos.y, -self.size.x, self.size.y)
        end
...
function KaizoBouncyBean:destroy()
    self.marked_for_deletion = true
end

function KaizoBouncyBean:do_collision()
...
function KaizoBouncyBean:handle_collision(collide, pos2, size2, ent)
...
function KaizoBouncyBean:SaveState()
    return {
        name = self.name,
...
function KaizoBouncyBean:LoadState(state)
...
function KaizoBouncyBean:HandleProperty(prop)
    if prop.name == "dir" then
        self.dir = prop.value
    end
end

function KaizoBouncyBean:HandlePlayerCollision(player, collide)
